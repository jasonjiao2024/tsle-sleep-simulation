...
def compute_session_metrics(session_df: pd.DataFrame) -> Dict[str, float]:
    ...
    Primary outcomes (v2 -- SWA-based, NREM-gate-weighted):
    - session_swa_enhancement: NREM-gated SWA enhancement vs baseline
    - final_swa: NREM-gated mean SWA in last 5 min (10 epochs)
    - cumulative_swa: NREM-gated time-integral of SWA(t) over session
    - mean_swa_nrem_weighted: NREM-gate-weighted mean SWA across session
    ...
    NREM gate weighting: The NREM gate drops to zero in late epochs
    (REM transition). Averaging SWA over the full session mixes high-
    power NREM epochs with near-zero REM epochs, creating an artifact
    where conditions look similar because REM dominates the average.
    NREM-gate-weighting ensures only epochs where entrainment is
    physiologically possible contribute to the metric.
    ...
    # NREM gate weights for averaging (avoid dividing low-gate epochs
    # equally with high-gate epochs)
    has_nrem_gate = 'nrem_gate' in session_df.columns
    if has_nrem_gate:
        nrem_weights = session_df['nrem_gate'].values.copy()
        # Floor at a small epsilon so all epochs contribute minimally
        # (prevents division by zero in edge cases)
        nrem_weights = np.maximum(nrem_weights, 0.01)
        nrem_weights_final = nrem_weights[-n_final:]
    else:
        nrem_weights = np.ones(len(session_df))
        nrem_weights_final = np.ones(n_final)

    # Primary outcomes (v2 SWA) -- NREM-gate-weighted
    if 'swa' in session_df.columns:
        swa_vals = session_df['swa'].values
        swa_enh_vals = session_df['swa_enhancement'].values

        # NREM-gate-weighted session SWA enhancement
        # This ensures low-gate (REM) epochs do not dilute the metric
        w_total_final = float(np.sum(nrem_weights_final))
        if w_total_final > 0:
            session_swa_enhancement = float(
                np.sum(swa_enh_vals[-n_final:] * nrem_weights_final) / w_total_final
            )
            final_swa = float(
                np.sum(swa_vals[-n_final:] * nrem_weights_final) / w_total_final
            )
        else:
            session_swa_enhancement = 0.0
            final_swa = 0.0

        # NREM-gate-weighted cumulative SWA
        cumulative_swa = float(
            np.trapezoid(
                swa_vals * nrem_weights,
                session_df['time_sec'].values,
            )
        )

        # Session-wide NREM-weighted mean SWA
        w_total = float(np.sum(nrem_weights))
        mean_swa_nrem_weighted = float(
            np.sum(swa_vals * nrem_weights) / w_total
        ) if w_total > 0 else 0.0
    else:
        final_swa = 0.0
        cumulative_swa = 0.0
        session_swa_enhancement = 0.0
        mean_swa_nrem_weighted = 0.0
    ...
    metrics = {
        # Primary (v2 SWA, NREM-gate-weighted)
        'session_swa_enhancement': session_swa_enhancement,
        'final_swa': final_swa,
        'cumulative_swa': cumulative_swa,
        'mean_swa_nrem_weighted': mean_swa_nrem_weighted,
        # Primary (legacy SDR)
...
def aggregate_protocol_results(
    ...
    # Reorder columns (filter to only those actually present in the DataFrame,
    # since paired_swa_enhancement is added later in the pipeline)
    id_cols = ['subject_id', 'condition']
    primary_cols = ['session_swa_enhancement', 'paired_swa_enhancement',
                    'final_swa', 'cumulative_swa',
    ...
    all_desired = id_cols + primary_cols + secondary_cols
    present_ordered = [c for c in all_desired if c in result_df.columns]
    other_cols = [c for c in result_df.columns if c not in all_desired]
    result_df = result_df[present_ordered + other_cols]
