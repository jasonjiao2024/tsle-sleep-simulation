...
def rfig6_sham_validation(session_metrics_df: pd.DataFrame):
    ...
    # Use PAIRED SWA enhancement (primary metric, correct sham hierarchy)
    # instead of session_swa_enhancement (legacy within-session metric
    # where the hierarchy is inverted). Fixed to use
    # figure verification found rfig6 was showing the wrong metric.
    metric_col = 'paired_swa_enhancement' if 'paired_swa_enhancement' in session_metrics_df.columns else (
        'session_swa_enhancement' if 'session_swa_enhancement' in session_metrics_df.columns else 'session_sdre'
    )
    metric_label = 'Paired SWA Enhancement (%)' if metric_col == 'paired_swa_enhancement' else (
        'SWA Enhancement (%)' if metric_col == 'session_swa_enhancement' else 'Session SDRE'
    )
