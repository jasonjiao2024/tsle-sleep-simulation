"""
SO-Phase Excitability Gating — Extended Sensitivity Sweep.

Extends the original sweep to higher so_modulation values (1.5, 2.0)
to test whether stronger gating closes the continuous-pulsed gap and
reduces the sham artifact.

At so_modulation=2.0, lambda_eff drops to 0 during SO down-states,
representing near-complete cortical silencing — realistic for deep NREM.

Usage:
    python scripts/run_so_gating_sensitivity_extended.py --workers 6
    python scripts/run_so_gating_sensitivity_extended.py --workers 6 --n-subjects 208
"""
...
# Extended sweep: only the higher values
SO_MOD_VALUES = [2.0]


def load_subjects(n_subjects: int = 208) -> Dict[str, pd.DataFrame]:
...
def main():
    parser = argparse.ArgumentParser(
        description='SO-Phase Gating Extended Sensitivity Sweep (1.5, 2.0)'
    )
    parser.add_argument('--workers', type=int, default=None)
    parser.add_argument('--n-subjects', type=int, default=208)
    ...
    logger.info("=" * 70)
    logger.info("SO-PHASE GATING — EXTENDED SENSITIVITY SWEEP (1.5, 2.0)")
    logger.info("=" * 70)
    logger.info(f"  Subjects: {len(subjects)}")
    logger.info(f"  SO modulation values: {SO_MOD_VALUES}")
    ...
    results_df.to_csv(RESULTS_DIR / 'sensitivity_sweep_extended.csv', index=False)
    ...
    agg_df.to_csv(RESULTS_DIR / 'sensitivity_summary_extended.csv', index=False)
    ...
    for _, row in agg_df.iterrows():
        print(f"\nso_modulation = {row['so_modulation']}")
        for cond in KEY_CONDITIONS:
            sdre_key = f'{cond}_mean_sdre'
            swa_key = f'{cond}_mean_swa_enh'
            sdre = row.get(sdre_key, float('nan'))
            swa_e = row.get(swa_key, float('nan'))
            print(f"  {cond:<30}: SDRE={sdre:+.4f}  SWA_enh={swa_e:+.1f}%")
        print(f"  Continuous-Pulsed gap (SDRE):  {row['continuous_pulsed_gap']:+.4f}")
        print(f"  Continuous-Pulsed gap (SWA):   {row.get('continuous_pulsed_swa_gap', 0):+.1f}%")
        print(f"  Better Sham effect:            {row['better_sham_effect']:+.4f}")
