...
from analysis.wc_thalamocortical_model import compute_sdr, compute_swa
...
def define_redesigned_protocols(
    ...
    # 7. Sham (muted yoked): identical to fixed_delta_pulsed in every
    # respect EXCEPT that the forcing amplitude is zero. The closed-loop
    # SO phase detector and the stim trigger logic still run, but no
    # acoustic energy is delivered. This is the field-standard sham
    # design (Papalambros 2017; Choi 2020): it isolates the audio-induced
    # perturbation per se from the closed-loop acoustic pacemaker
    # phenomenon. F=0 strictly, not subthreshold.
    protocols['sham'] = {
        'phases': [{'freq': 2.0, 'duration_sec': session_duration_sec, 'name': 'sham_muted_yoked'}],
        'stim_mode': 'pulsed',  # detector runs, F=0
        'description': 'Muted yoked sham (pulsed detector, F=0)',
        'forcing_override': 0.0,
    }
    ...
    # 12. Adaptive protocol: TRUE online controller.
    # Replaces the prior static-baseline-split design (which picked one
    # of two open-loop plays at session start and never adapted).
    # The new design slices the session into 5-min "blocks" and asks the
    # runner to pick each block's frequency from a small frequency menu
    # based on the running PLV / SWA from the previous block. The runner
    # implements the closed-loop logic; here we just declare the structure
    # so the protocol parser knows it's a special adaptive condition.
    adaptive_block_sec = 300.0  # 5-minute blocks
    n_blocks = max(1, int(round(session_duration_sec / adaptive_block_sec)))
    adaptive_phases = []
    for b in range(n_blocks):
        block_dur = adaptive_block_sec if b < n_blocks - 1 else \
            (session_duration_sec - adaptive_block_sec * (n_blocks - 1))
        adaptive_phases.append({
            'freq': 2.0,  # initial frequency; runner may override
            'duration_sec': block_dur,
            'name': f'adaptive_block_{b}',
            'adaptive': True,
        })
    protocols['adaptive_protocol'] = {
        'phases': adaptive_phases,
        'stim_mode': 'adaptive',
        'description': 'Adaptive online controller (5-min PLV-guided blocks)',
        'adaptive_choice': 'online',
        'adaptive_freq_menu': [2.0, 1.5, 0.85, 4.0],  # menu of candidate freqs
        'adaptive_block_sec': adaptive_block_sec,
    }
...
def compute_extended_metrics(session_df: pd.DataFrame) -> Dict[str, float]:
    ...
    # ── Tier-4 CLAS outcome metrics (session-level) ──
    # The model writes the final session-level CLAS metrics to the LAST
    # row only (it computes ERP/spindle/KC stats once at session end so
    # the per-epoch loop is fast). Read the last-row values directly.
    clas_keys = [
        'erp_so_amplitude_uv', 'spindle_rms', 'slow_wave_slope',
        'kc_density_per_min', 'pulse_phase_concentration',
    ]
    if len(session_df) > 0:
        last_row = session_df.iloc[-1]
        for k in clas_keys:
            v = float(last_row.get(k, 0.0)) if k in session_df.columns else 0.0
            metrics[f'mean_{k}'] = v
            metrics[f'max_{k}'] = v
        metrics['n_pulses_total'] = int(last_row.get('n_pulses_total', 0)) \
            if 'n_pulses_total' in session_df.columns else 0
    else:
        for k in clas_keys:
            metrics[f'mean_{k}'] = 0.0
            metrics[f'max_{k}'] = 0.0
        metrics['n_pulses_total'] = 0
